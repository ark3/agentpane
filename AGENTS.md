...
- **All three CLIs run on the home server, each pinned to one model.**
  ...
  Pi reads its own default from `~/.pi/agent/settings.json`, which the owner may edit at any time — measured there on 2026-09-13, after the sandbox was widened to mount that directory read-write, `get_state` reported `deepseek/deepseek-v4.1-flash` at `thinkingLevel: "high"` both directly and through `sbox` (`docs/MANUAL_TESTING.md`, "Pi arrives on the home server").
  Earlier that same day the directory was read-only, and Pi — unable to take the lock it needs merely to *read* that file — resolved no model at all, answering `get_state` with `"id": "unknown"`.
  That is the failure the flag exists to make impossible, and it is why the flag stays mandatory even now that the file is readable.
...
- Pi fork behaviour, first measured on the work laptop (2026-08-20, `pi 0.84.2`) and re-measured on the home server (2026-09-15, `pi 0.85.1`, on `deepseek/deepseek-v4.1-flash`): forking at a user message is exclusive of that message, and forking during a streaming turn succeeds while the turn stops — `isStreaming` goes false, `agent_settled` arrives with no assistant text, and the file the turn was streaming into ends with that turn's user message and an assistant entry carrying no text.
  The 0.84.2 run did not earn even that much: it forked at the first user message, where exclusivity empties the new branch whatever became of the turn, and it never opened the streamed-into file.
  Neither run shows that streamed text was *discarded*: the Pi cell gates only on `agent_start`, with no delta threshold and no count of what had streamed, so on a reasoning model a fork ~2s in cannot tell a discarded reply from one not yet begun.
  The 0.85.1 run also read the fork's moved-to session file as already on disk when `fork` returned, carrying the rewound prefix — one sample at the instrument that had once read `false`.
