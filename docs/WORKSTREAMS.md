...
| client-shell | **offline verified, and hand-tested live in a browser** (open: OW-24, OW-42, OW-48, OW-49, OW-56, OW-60, OW-61, OW-62) | main |
...
| OW-60 | defect | `src/client/App.svelte`, `src/client/app.css`, `src/client/render/Message.svelte`, `src/client/App.test.ts`, `e2e/follow.spec.ts` | **The transcript navigation affordance is anchored to the scrolled conversation instead of the pane viewport, and it needs to become a right-rail navigation pill.** Today the only affordance is the `showJumpToLatest` button rendered inside `.conversation` (`App.svelte`, `.jump-to-latest` in `app.css`), so when the reader is mid-history it can scroll off-screen with the content it is supposed to help navigate. Replace it with one always-present, tall narrow pill fixed to the right of the conversation pane, outside the bordered transcript so it never obscures content; localhost-only and owner-only means no responsive/discoverability work is required here. Four stacked segments, icon-only: double-chevron up = start, single-chevron up = previous user input, single-chevron down = next user input, double-chevron down = end. `Prev`/`next` are relative to the first user turn at or below the viewport top, not "most visible"; `start` and `end` mean true top/bottom of the scroller. Grounding for the executor: user turns already render as DOM landmarks with stable `data-index` and `data-role="user"` in `Message.svelte`, and `transcript.ts` preserves those original indices even in reading view specifically so shell scroll logic can keep targeting them. Keep all navigation logic in the shell beside `handleConversationScroll`/`jumpToLatest`; these jumps are manual reading actions, so like a manual scroll they must never arm follow mode. Done when jsdom pins the button-state and target-selection logic (including disabled states at the ends and around consecutive assistant/tool messages), and `bun run test:browser` is extended or replaced with a real-browser check that the right-rail control stays visible while the conversation scrolls and that its jumps land on the intended user turns without regressing follow-mode behavior. |
| OW-61 | defect | `src/client/render/Message.svelte`, `src/server/adapters/codex/adapter.ts`, `src/server/adapters/codex/reducer.ts`, `src/server/adapters/types.ts`, `resources/codex-protocol/v2/Thread{Start,Resume}Response.ts` | **Assistant-turn metadata shows model but drops Codex reasoning effort, so the transcript cannot display the effort level beside the model name.** The source exists on the Codex side today: `thread/start` and `thread/resume` responses both carry `reasoningEffort`, but the adapter/reducer identity path only retains `model`/`modelProvider`, and `Message.svelte` renders only `message.model`. The agreed scope is deliberately narrow: when effort is present, render it alongside the model in the assistant-turn meta; when it is absent, omit it with no placeholder. Do not chase mid-session settings churn here — this UI does not let the owner change model or effort, so capturing the value on attach/start is enough unless source inspection proves otherwise. Grounding for the executor: Pi's currently transcribed RPC protocol (`src/server/adapters/pi/protocol.ts`) exposes no corresponding effort field, so this is expected to be Codex-only unless further verification finds otherwise. Done when the shared/server types can carry an optional effort value to the client, Codex start/resume plumbs it through without regressing existing model stamping, and tests pin both rendering cases: model-plus-effort when present and model-only when absent. |
| OW-62 | defect | `src/client/render/markdown.ts`, `src/client/render/Markdown.svelte`, `src/client/render/*.test.ts`, `src/server/http/static.ts` | **Repo/local file references in markdown are rendered as ordinary links even though agentpane has nowhere useful to send them.** Today the sanitizer hook in `markdown.ts` treats every non-`#` anchor as an external link (`target="_blank"`), so references like `src/client/api.ts:35` or `/home/asa0717/src/agentpane/src/client/api.ts:35` become clickable. In practice those hrefs fall into the SPA static handler in `static.ts`, which serves `index.html` for a navigation request, so clicking them effectively reloads the app. The result is a false affordance *and* it drops important visible context: the rendered label can collapse to the file name while hiding the line suffix the reader actually needs. The agreed fix is deliberately not a file viewer: detect local/repo file references in rendered transcript markdown, preserve `path:line` in the visible text, style them distinctly from prose, but render them as non-link inline references so ordinary text selection keeps working. Real `http(s)` links remain real links. Done when tests pin both behaviors: local file refs no longer produce `<a>` tags or navigable hrefs, while true web URLs still do; and a browser/manual check confirms clicking a file reference no longer reloads the app. |
