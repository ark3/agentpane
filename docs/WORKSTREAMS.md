...
- **A client keys what it holds for a live session by its `handle`**, which every per-session event carries beside the ref and a rename never moves (D24, OW-suyinu), and takes the ref from the events as an attribute that may change under it.
  ...
  agentpane-mode does the same since OW-danifa, taking the ref from any notification under the handle; the one notification that reaches a buffer before it holds the handle is the snapshot of an attach answered under another ref than it asked for, which the helper therefore sends after the attach's reply, since the reply is what names the handle (`sessions/attach` in `src/emacs/helper.ts`, `agentpane--notified-buffer` in `emacs/agentpane.el`).
