...
- **A client keys what it holds for a live session by its `handle`**, which every per-session event carries beside the ref and a rename never moves (D24, OW-suyinu), and takes the ref from the events as an attribute that may change under it.
  ...
  agentpane-mode does the same since OW-danifa, taking the ref from any notification under the handle; the one notification that can reach Emacs before the buffer that asked for it holds the handle is the first snapshot of an attach answered under another ref than it asked for, and, where no other attachment already holds that handle, the helper tags it with `askedFor`, the ref that attach asked for, by which `agentpane--notified-buffer` in `emacs/agentpane.el` finds the buffer that asked, which takes the handle from it.
