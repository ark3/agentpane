...
- **A client keys what it holds for a live session by its `handle`**, which every per-session event carries beside the ref and a rename never moves (D24, OW-suyinu), and takes the ref from the events as an attribute that may change under it.
  The browser's reducer does (OW-kimaya): an event under a known handle whose `session` is a new ref moves the view, the summary carrying that handle and the selection onto it.
  agentpane-mode does the same since OW-danifa, taking the ref from any notification under the handle; the one snapshot that can reach Emacs before the buffer that asked for it holds the handle is that of an attach answered under another ref than it asked for, and agentpane-mode keeps it under its handle until the attach's reply names that handle (`agentpane--unclaimed-snapshots` in `emacs/agentpane.el`).
  It cannot rely on handling the reply first: as of jsonrpc.el 1.0.29 on Emacs 31.1, a reply to an asynchronous request that arrives while a synchronous one is outstanding is held back as an "anxious continuation" until that one returns, while notifications arriving meanwhile are handled at once.
