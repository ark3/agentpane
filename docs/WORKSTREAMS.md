...
| OW-63 | feature | `src/client/render/Block.svelte`, `src/client/render/Markdown.svelte`, `src/client/render/Thinking.svelte`, `src/client/render/tools/Output.svelte`, `src/client/render/types.ts` (`resultText`), `src/client/render/*.test.ts` | **Nothing in the transcript can be copied without a manual text selection — add a one-click copy affordance at the block level.** **Copy operates on individual blocks, never on a whole message.** An assistant message is an ordered mix of text/thinking/toolCall blocks (`ContentBlock` in `types.ts`); concatenating those into one "copy the message" string was considered and rejected — it would invent a serialization (how to render a toolCall as text?) and produce something no single source ever held. So the copyable units are the blocks themselves: a **fenced code block** (its raw `block.text`/code source, not the highlighted HTML and not the fence's language tag), a **markdown text block** (its source markdown), a **thinking block** (`block.thinking`), and a **tool result / command output** (`resultText(message)` for a `toolResult`, the `Output.svelte` `text` prop for command output). What reaches the clipboard is the **source prop the component already holds**, not the rendered DOM — so it is precise (only that block, no adjacent chrome like the tool name, status pill, streaming cursor, or a truncation notice), it is the real bytes rather than the browser's whitespace reconstruction of the layout, and it is **not limited to what is rendered** (`Output.svelte` truncates at 20000 chars for display; copy still yields the full source). That completeness-and-precision over drag-selection is the whole point of the button. **No images** — image blocks and the image parts of a tool result get no copy control; text copy copies text. **Empty/redacted blocks get no button** — a thinking block with no text (Pi emits these with an opaque signature) has nothing to copy, so the control should not render. Feedback is inline on the button: a brief ✓ on success and ✗ on failure via a transient class/state, no global toast layer. `navigator.clipboard.writeText` is the mechanism; guard for its absence (undefined in jsdom and on a non-secure origin) rather than throwing. Deliberately **read-only**: copy only, no edit/resubmit (blocked on OW-22). Load-bearing: block-level not message-level, copies source not rendered DOM, no images, no button when there is nothing to copy. Incidental: exact icon, exact ✓/✗ glyphs, button placement within each block's chrome. Done when a vitest/testing-library test per block type asserts that clicking its copy control calls `writeText` with the expected source string (mock the clipboard), that an empty/redacted thinking block renders no control, and that a rejected write surfaces the failure state rather than throwing. |
| OW-64 | feature | `src/client/render/Message.svelte`, `src/client/render/tools/Output.svelte`, `src/client/App.svelte`, `src/client/app.css`, new overlay component under `src/client/render/` or `src/client/`, `src/client/render/*.test.ts` or `src/client/App.test.ts` | **Long messages and long command output cannot be read without scrolling the transcript — add a first-class expand-to-overlay viewer.** An expand control on the same readable units as OW-63 (message, tool result / `Output.svelte` body, fenced code block) opens a full-screen overlay showing that unit's content large. The overlay is the app's first modal: there is none today (`App.svelte`'s only layered element is `.jump-to-latest`, and errors render inline, not as a dialog), so this row introduces a small reusable one. Hard requirements, all agreed with the owner: (1) the body is **real, selectable text** — click-and-drag selection plus the browser's own copy menu/hotkey must work, so it renders actual sanitized content (reuse `renderMarkdown`/`renderCode` from `markdown.ts`; never bypass the D5 sanitize choke point) and never a `pointer-events:none` node or an image; (2) a **copy-all button** in a corner copies the whole block in one click (share OW-63's copy helper/feedback); (3) a **visible close affordance** — an X in a corner — plus **Esc**; (4) **no dismiss-on-backdrop-click**, because a text selection that ends on the backdrop would otherwise be read as a dismiss and lose the selection. On open, move focus into the panel; restore it on close. Any chrome (the copy/expand/close buttons themselves) must be excluded from what the copy-all copies and from the selectable body. Load-bearing: selectable real text, no backdrop-dismiss, explicit X + Esc, sanitize path preserved. Incidental: overlay's exact layout, animation, which file the component lives in. Done when a test opens the overlay from a unit's expand control, asserts the rendered body carries that unit's content, asserts Esc and the X close it while a simulated backdrop click does **not**, and asserts copy-all writes the full source (mock the clipboard as in OW-63). |
