...
| OW-50 | change | `src/client/App.svelte:545-561`, `src/client/app.css:185-198`, `src/client/render/Message.svelte:71` | **The read-only preview is a second transcript renderer, and it has already drifted from the first.** `app.css:186-197` re-implements `Transcript.svelte`'s container plus `Message.svelte`'s `.msg`/`.user`/`.who` rules with the same tokens and the same values, and two divergences already exist: the preview labels both roles ("You"/"Agent", `App.svelte:551`) where the transcript labels only the user (`Message.svelte:44,52`), and `.transcript` sets font-family, size, line-height, colour and background (`Transcript.svelte:34-44`) where `.preview` sets none of them and inherits. Fix it **client-side only** — the server keeps returning `SessionPreviewTurn[]`. Do *not* widen `SessionPreviewResponse` to carry `AgentMessage`s: that is a D11 frozen-interface change, and its Codex half needs a whole second mapper, because stored rollout records are `response_item` payloads rather than the `ThreadItem`s `mapping.ts` consumes (their discriminants differ — rollout `item_completed` carries `"UserMessage"` against ThreadItem's `"userMessage"` — and the rollout's `event_msg` stream is incomplete anyway: the one local corpus file has `item_completed` for user messages and compaction but none for assistant messages). Instead map each turn to a message-shaped object in the client, render it through `Transcript`, and delete `App.svelte:548-558` and `app.css:186-197`. A synthetic assistant message typechecks without lying about a provider: `Api` and `ProviderId` are open unions (`node_modules/@earendil-works/pi-ai/dist/types.d.ts:16,20`), so `provider` and `model` carry the backend id (`pi`/`codex`) — the owner's ask, since that is the one identity a preview genuinely knows — and `api` a placeholder; `stopReason:"stop"` with zero usage suppresses both the error banner and the token line (`Message.svelte:63-79`). User turns need no fabrication at all (`UserMessage` is `{role,content,timestamp}`). One consequence to handle: the meta line renders only when `usage.totalTokens > 0` (`Message.svelte:71`), so the backend id would never appear — split that condition so the model span renders whenever there is a model and the token/cost spans only when there is usage, which also unhides the model name for a real turn that reports zero usage. Keep `.preview-empty`'s wording: "no readable transcript to preview" is more accurate than the transcript's "No messages yet", because the server's text-only extraction can come up empty on a session that has plenty in it. **Success criterion: the preview and an attached session render the same DOM structure for the same content.** This is a de-duplication, not a redesign; its two visible consequences are that assistant turns lose the "Agent" label and the preview picks up the transcript's typography. Decided with the project owner on 2026-08-15; read from source, nothing run. |
| OW-51 | change | `src/client/render/transcript.ts`, `src/client/App.svelte` | **A "reading view" toggle that elides tool calls, tool results and thinking blocks, for scrolling back through a session while it is still running.** Add a pure `condense(messages)` beside `buildTranscript` (`transcript.ts:45`) that drops `toolResult` messages and `toolCall`/`thinking` blocks and keeps user and assistant text. Keep each entry's *original* index so follow mode's `data-index` anchor still resolves (`App.svelte:219`); the anchor is always a user message (`App.svelte:340`), which condensing never elides, so OW-47's scroll work is untouched. Decisions, all taken with the project owner on 2026-08-15: **the name is "reading"** — an established convention for strip-the-chrome-show-the-prose, and it names a view state; "focus" was rejected for colliding with DOM focus (`App.svelte:452`), "review" for naming an activity. **One global boolean, not persisted, not per-session** — nothing in `src/client` touches `localStorage` today, and this should reset on reload like the workspace and backend selects. **The control goes in `.session-controls`** beside Workspace/Backend/New; there is no conversation header and inventing one is out of scope. **Scroll position may jump when toggling** — content height changes a lot, and pinning the topmost visible user message is possible with `reconcile()`'s existing machinery but is not wanted until someone is actually annoyed. **The toggle stays visible and inert while previewing**, where after OW-50 it is a no-op; a control that appears and disappears on attach is worse. **Hiding thinking blocks is a first cut, not a settled decision** — the original ask was "tool stuff", `Thinking.svelte` already collapses thinking once a turn settles so keeping it would cost roughly one line per turn, and the owner wants to use the feature before deciding. Expect this row's shape to change after first use. No liveness cue is needed: the Abort button renders only while streaming (`App.svelte:583`) and the sidebar row carries a ● (`App.svelte:525`), neither of which reading view touches. Touches neither `.conversation` nor the scroll code, but it changes content height while follow can be armed — run `bun run test:browser` (AGENTS.md, OW-49). |
| OW-52 | change | `src/client/render/Message.svelte:44` | **Drop the visible "You" label from user turns.** The filled surface, accent left border and width cap (`Message.svelte:114-122`) already mark a user turn as the reader's own; the word is redundant. Applies to the preview too, which after OW-50 renders through this same component. **The `<span class="who">` is currently the only role signal for assistive technology** — `data-role` is not announced, the `<article>` carries no `aria-label`, and assistant turns have no cue at all — so removing it leaves the transcript role-less to a screen reader. **The project owner accepted that explicitly on 2026-08-15**, recorded here so a future agent reads it as decided rather than as a bug to helpfully fix. Revisit if anyone but the owner uses this. |
...
| OW-54 | question | `docs/TRACKING.md` | **Whether to replace this table with one-file-per-row storage, and whether to build that or adopt beads.** Sketched in `docs/TRACKING.md` — the measurements behind it, the Maildir-shaped design, the tool surface, and five open questions. Not restated here; read the doc. A separate project from the agent UI that happens to live in this repo, so it competes with nothing on this list. Settling it means a `D` decision in `DESIGN.md` and a rewrite of `AGENTS.md` and both skills. Do not act on the doc without the owner saying so. |
