...
| OW-46 | defect | `src/client/session-state.ts:85-108`, `src/server/http/session-manager.ts:231-271`, `src/server/adapters/pi/process.ts:283-313` | **A new session's list entry stays on its temporary `virtual:<uuid>` label after the first prompt and never catches up to the real preview without a full page reload.** Reported live: create a session, submit the first prompt, and the sidebar entry keeps showing the virtual placeholder — reloading the browser (a fresh `listSessions()` call from `start()`) is what finally shows the real one. Two things are established by reading the code, and one is not: (1) **Established** — the `"renamed"` reducer (`session-state.ts:99-101`) only patches `.ref` on the matching summary; it never touches `.preview`, so the local copy of that summary can never itself grow a real preview, only ever fall back to `` `${backend} ${id}` `` (`App.svelte:460`) with whatever id it has. (2) **Established** — the only re-list of the session index tied to this rename is the single `sessionsChanged()` broadcast fired synchronously inside `#adoptRef` (`session-manager.ts:270`, called from `submit()`'s `finally` at line 238), which the client answers with exactly one `refreshSessions()` fetch (`controller.ts:108-126`, `session-state.ts:70`); nothing re-triggers another list fetch for this session afterward — no event fires on turn completion, only on create/attach/rename/close (`session-manager.ts:185,218,270,386,445`, `app.ts:246`). So whatever that one fetch returns is what the sidebar is stuck with until something unrelated (another create/attach/close elsewhere, or a reload) happens to trigger a fresh list. (3) **Not established, and load-bearing** — whether the Pi session file already contains the user's first message by the time that one `sessionsChanged()` fires. `submit()` (`pi/process.ts:283-295`) awaits the `prompt` RPC's response, and only then (in `#adoptRef`) is `sessionsChanged()` sent; if Pi's response to `prompt` is returned before (or concurrently with) the message actually being flushed to the JSONL file, the one refetch reads a file with no user turn yet — `parsePiSession`'s preview scan (`pi.ts:69-94`) then finds nothing, `preview` stays `null`, and there is no second chance. Verify this against a live or captured Pi session (does `sessionFile` in the `get_state` response at `pi/process.ts:312-313` point to a file that already has the user message on it the moment `submit()` resolves?) before deciding the fix — if the file is already complete by then, the bug is client-side only (case 1/2 above: patch `.preview` too, or add a retry/second refresh); if it is a genuine write-ordering race, the fix belongs on when `sessionsChanged()` fires, not just how the client reacts to it. Done when: a reproduction (live or a controller/session-manager integration test simulating the real SSE sequence — `sessions-changed`, `renamed`, `snapshot`, in that order per `broadcaster.ts:137-147`) shows the sidebar entry for a freshly-created, freshly-prompted session displaying real preview text without any full-client restart, and a test pins whichever side (client reducer, or server trigger timing) turned out to be the actual cause. |
