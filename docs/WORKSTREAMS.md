...
| OW-63 | feature | `src/client/render/Message.svelte`, `src/client/render/Thinking.svelte`, `src/client/render/tools/ToolCard.svelte`, `src/client/render/tools/Output.svelte`, `src/client/render/types.ts` (`resultText`), `src/client/render/*.test.ts` | **Nothing in the transcript can be copied without a manual text selection — add a one-click copy affordance to each readable unit.** The units are: an assistant or user message (`Message.svelte`'s `.msg` articles), a thinking block (`Thinking.svelte`), a tool result / command output (`ToolCard.svelte` chrome, whose body text is `resultText(message)` from `types.ts` for a `toolResult` and the `Output.svelte` `text` prop for command output), and each fenced code block. What gets copied is the block's **source text**, never the rendered HTML — the plain markdown/text/code the agent produced, which every one of these components already receives as a prop (`message.content`, `text`, `resultText(...)`), so no re-extraction from the DOM. Feedback is inline on the button itself: a brief ✓ on success and ✗ on failure via a transient class/state, no global toast layer. `navigator.clipboard.writeText` is the mechanism; guard for its absence (it is undefined in jsdom and on a non-secure origin) rather than throwing. Deliberately **read-only**: copy only, no edit/resubmit (that path is blocked on OW-22's fork semantics). Load-bearing: copies source not HTML, and feedback stays local to the button. Incidental: exact icon, exact ✓/✗ glyphs, button placement within each unit's chrome. Done when a vitest/testing-library test per unit type asserts that clicking its copy control calls `writeText` with the expected source string (mock the clipboard), and that a rejected write surfaces the failure state rather than throwing. |
