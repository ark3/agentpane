...
Since OW-danifa neither alone strands anything in agentpane-mode, and since OW-gusaru both in one outage do not either: a buffer holds the handle and takes its ref from every notification under it.
...
Until OW-gusaru the helper moved an attachment by comparing the snapshot's ref with the one it had last told Emacs, so after a rename it never heard and a re-attach elsewhere it dropped the opening snapshot under the new handle, and the buffer froze.
...
What the helper still cannot follow is a rename before a close elsewhere and a re-attach by the new ref after it: nothing tells the helper a handle is gone, and once the closed container is gone no name links the ref the buffer holds to the one the rename gave it, so that buffer counts itself attached with a frozen transcript.
...
D21's reconnect gap closed in the browser when OW-kimaya keyed its views by the handle: a rename missed while the stream was down no longer strands a view, since the opening snapshot under the handle carries the current ref, and a re-attach elsewhere in that window leaves one view of the ref, under the new handle; in agentpane-mode it closed with OW-danifa, whose buffers the same snapshot moves, and with OW-gusaru where a rename and a re-attach elsewhere fall in one outage, the helper asking the server which handle the ref it told Emacs names now (D21).
