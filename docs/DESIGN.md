...
So `CodexReducer.hydrate` now takes the running turn from the listing, its streaming state, a compaction when it lists no user message, and its id, which the adapter adopts as though `turn/started` had arrived, so `abort()` interrupts it, `submit()` steers it and `compact()` refuses; a turn the stream ended while the history was paged in stays ended.
