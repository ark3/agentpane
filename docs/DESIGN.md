...
Codex has no fork that keeps no turn yet: at its first fork point the adapter sends `thread/fork` no `lastTurnId`, which as of `codex-cli 0.156.0` kept the parent's whole history (OW-sayaju's run, whose one-turn parent's fork named its base at the end of that turn), so the borrower reads the parent's last `turn_context`; a fix that makes that fork keep nothing (OW-hojefo) inherits this rule, since the borrower would otherwise run at the pair `thread/resume` answers, `config.toml`'s.
