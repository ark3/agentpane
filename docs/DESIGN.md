...
Since OW-mofuho no event says a rename on either wire: the server sends a snapshot under the handle carrying the new ref, and for an attach answered under another ref than it asked for, agentpane-mode keeps that snapshot under its handle until the attach's reply names the handle to the buffer that asked; until then the helper's `session/renamed` had been that buffer's only link from the asked-for ref to the handle.
The snapshot cannot simply follow the reply on the wire: as of jsonrpc.el 1.0.29 on Emacs 31.1, a reply to an asynchronous request that arrives while a synchronous one is outstanding is held back as an "anxious continuation" until that one returns, and notifications arriving meanwhile are handled at once, so agentpane-mode can handle the reply after a notification written after it (reproduced 2026-09-25 with a fake helper).
