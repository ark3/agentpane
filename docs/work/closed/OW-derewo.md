...
Implemented server-owned conversation model reporting across all three adapters, snapshot/status events, and client session state; the composer now lists and sets live backend models only before the first message, then renders the actual running model as a label. Filtered model-list failures surface while merged listing stays tolerant. Request ordering covers late snapshots, reselection, session renames, concurrent prompts, and authoritative null model updates without optimistic client model truth. Adapter, controller, reducer, API, rendering, and browser layout regressions were shown red before green. Verified on landed main with bun run check (48 files, 928 tests) and bun run test:browser (20 tests). Real app-driven turns on the home server selected Claude Haiku and Codex gpt-5.6-luna and confirmed those models in the locked label/footer; the evidence and retired stale claims are in docs/MANUAL_TESTING.md. OW-9 now records why Codex reducer identity lag cannot stale the picker.
