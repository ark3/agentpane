...
Added a non-persistent System/Light/Dark theme selector to the masthead, resolved the system preference to a concrete document `data-theme` before mount, tracked live system changes only in System mode, and moved the palettes plus `color-scheme` into attribute-selected CSS. Production CDP captures found a 156–180 ms white frame in five of five dark cold loads, so the documented no-attribute dark media fallback was added; five repeat captures showed no white frame. Red-first jsdom and sabotaged Playwright checks proved the control/state logic and computed dark token assertion. `bun run check` passed 899 tests, the ordered follow file passed 8 tests, and `bun run test:browser` passed 18 tests. An independent pre-existing follow-frame race exposed by repeated browser runs is tracked as OW-gahoki.
