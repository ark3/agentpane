...
Done when a client that connects to a server holding a session with a pending request shows the blocked banner for it without a gesture, pinned by a test that goes red first.
If the decision instead is that these two fields are deliberately ephemeral, that closes this card too, recorded beside `SnapshotSource` in `src/server/http/broadcaster.ts` and in the OW-pezazo docblock in `src/client/session-state.ts` that currently points here.
