...
Second, the draw timer runs after the chunk's handlers not because its time is later: a timer activated during a pass of ripe timers waits for the next pass, whatever its time, as it would if each pass ran over a copy of `timer-list`; the C source is not on the home server, so that is inferred from this run, not read.
