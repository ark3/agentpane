...
| 42 | **Pi ran a shell tool and no approval dialog request reached the wire.** The turn produced `thinking` and `toolCall` blocks and settled with no `extension_ui_request` beside it. This was first claimed on `pi 0.84.1` from a field that structurally could not carry it — the probe snapshotted requests before the tool prompt was posted — and was measured for the first time on **`pi 0.85.1`**, home server, 2026-09-14, where the tool turn's own window was empty. Read it with both of its limits: the home server has no `trust.json` for the harness to copy, so this says Pi asked nothing *there* rather than that the sandboxed path never adds a prompt; and the field only ever sees an `extension_ui_request` carrying a dialog method, so empty means no dialog request reached the wire, never that Pi asked nothing | `--tool-check` run at `2760e01`; `agent_requests_seen.tool_turn` spans the tool prompt to that turn's recorded idle and is empty, with `checks.tool_output.turn_idle_at` closing it. See `docs/MANUAL_TESTING.md`, "The Pi smoke probe measures the tool turn's own requests" |
