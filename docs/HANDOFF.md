...
- **sbox** (the sandbox): `/home/asa0717/src/sandbox/sbox`.
  Has `pi` and `codex` profiles already (mounts `~/.pi/agent` / `~/.codex` rw), auto-detects the workspace (git root / marker), injects `codex --sandbox danger-full-access` so Codex doesn't run its own sandbox — but that flag is a no-op for `app-server`, which defaults threads to `read-only`; the effective policy is set by the adapter on each of `thread/start`, `thread/resume` and `thread/fork`, alongside `approvalPolicy: "never"` (OW-37, OW-18, D7a).
