...
| 42 | **Pi ran a shell tool with no approval dialog.** No `extension_ui_request` reached the wire; the turn produced `thinking` and `toolCall` blocks and completed. Note the harness copies `trust.json` into its temporary state dir where the machine has one — the home server had none on 2026-09-13 — so on the laptop this shows the sandboxed path does not *add* a prompt, not that Pi never asks | `--tool-check` run; `agent_requests_seen` empty — **and that field cannot carry this**: the probe snapshots it before the tool prompt is posted, so the tool turn is outside the window it covers. The finding is unmeasured, not disproved; OW-lapuye is the repair |
