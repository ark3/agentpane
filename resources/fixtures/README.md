...
The `set-model` scenario was captured for OW-hiligu on 2026-09-23 with Claude Code 2.1.280: two `set_model` control requests on a `--resume`, no turn, each answered by an `isReplay` user event ahead of its `control_response`; it needed no scrub, and the store lines a `set_model` leaves once a turn follows it are in the OW-hiligu section of `docs/MANUAL_TESTING.md`.
